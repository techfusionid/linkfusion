...
const initialBlocks: BentoBlockData[] = [
  { id: 'b1', type: 'link', content: 'Twitter / X', url: 'https://x.com', bgColor: '#e0f2fe' },
  { id: 'b2', type: 'link', content: 'GitHub', url: 'https://github.com', bgColor: '#f1f5f9' },
  { id: 'b3', type: 'text', content: 'Hey there! 👋 I build products at the intersection of design and code. Currently exploring AI, open source, and creative tools.', bgColor: '#ecfdf5' },
  { id: 'b4', type: 'image', content: 'Featured', bgColor: '#fef3c7' },
  { id: 'b5', type: 'link', content: 'Read My Blog', url: 'https://blog.example.com', bgColor: '#fce7f3' },
  { id: 'b6', type: 'heading', content: '🚀 Latest Projects', bgColor: '#f3e8ff' },
];
...
export function EditorProvider({ children }: { children: ReactNode }) {
  ...
  const resizeBlock = useCallback((id: string, w: number, h: number) => {
    setLayoutState(prev => prev.map(l => l.i === id ? { ...l, w, h } : l));
  }, []);
  ...
  const draftAwareAddBlock = useCallback((type: BlockType) => {
    const id = `b${Date.now()}`;
    const colors: Record<BlockType, string> = { link: '#e0f2fe', text: '#ecfdf5', image: '#fef3c7', heading: '#f3e8ff' };
    const defaults: Record<BlockType, string> = { link: 'New Link', text: 'Enter your text here...', image: '', heading: 'New Heading' };
    setBlocks(prev => [...prev, { id, type, content: defaults[type], bgColor: colors[type] }]);
    setLayoutState(prev => [...prev, { i: id, x: 0, y: Infinity, w: 2, h: 1, minW: 1, minH: 1 }]);
    setSelectedBlockId(id);
    if (isDraftMode) setHasUnpublishedChanges(true);
  }, [isDraftMode]);

  const draftAwareUpdateBlock = useCallback((id: string, updates: Partial<BentoBlockData>) => {
    setBlocks(prev => prev.map(b => b.id === id ? { ...b, ...updates } : b));
    if (isDraftMode) setHasUnpublishedChanges(true);
  }, [isDraftMode]);

  const draftAwareDeleteBlock = useCallback((id: string) => {
  ...
  const draftAwareUpdateProfile = useCallback((updates: Partial<ProfileData>) => {
    setProfile(prev => ({ ...prev, ...updates }));
    if (isDraftMode) setHasUnpublishedChanges(true);
  }, [isDraftMode]);

  const draftAwareSetLayout = useCallback((newLayout: LayoutItem[]) => {
